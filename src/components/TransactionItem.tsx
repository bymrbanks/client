...
function TransactionItem({ transaction }: { transaction: Transaction }) {
  const { deleteTransaction, updateTransaction } = useBudget();
    ...
    <div className="transaction__item">
        ...
        <div>
          <div className="transaction__name">
            {editMode ? (
              <input
                type="text"
                id={"name" + transaction.name + transaction.id}
              ...
              <label htmlFor={transaction.name + transaction.id}>
                {transaction.name}
              </label>
            )}
          </div>
      ...
      <div className="transaction__date">
        ...
        {editMode ? (
          <input
            type="date"
            name="date"
            value={updatedTransaction.date.toISOString().split("T")[0]}
            onChange={(e) =>
              setUpdatedTransaction({
                ...updatedTransaction,
          ...
          <label htmlFor="">{transaction.date.toDateString()}</label>
        )}
      </div>
      ...
      <div className="transaction__options">
        <div className="transaction__amount">
          {editMode ? (
            <input
              ...
              id={"amount" + transaction.name + transaction.id}
              onChange={(e) =>
                setUpdatedTransaction({
                  ...updatedTransaction,
        ...
        {!editMode && (
          <>
            <button
              className="btn transaction__edit--btn"
        ...
        {editMode && (
          <div className="transaction__edit">
            <button
              className="transaction__edit--confirm"
              onClick={() => {
                setEditMode(false);
                editTransaction();
              }}
            ...
            <button
              className="transaction__edit--cancel"
              onClick={() => {
                setEditMode(false);
